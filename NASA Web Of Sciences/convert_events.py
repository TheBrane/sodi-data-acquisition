...
def parse_conference_year(date):
...
def convert_articles(data, topic_key_val, link_key_val, researcher_topics, publication_topics, journal_topics, journal_links):
	article_topics = list()
	article_links = list()
...
def main():
	...
	article_topics, article_links, topic_key_val, link_key_val = convert_articles(data, topic_key_val, link_key_val, researcher_topics, publication_topics, journal_topics, journal_links)
	...
	for rowidx in range(len(data)):
		...
		if (not duplicate):
			...
			year = parse_conference_year(publication_row['CY'])
			link_key = 'L' + str(link_key_val)
			# Increment key value for next link
			link_key_val = link_key_val + 1
	...
	new_topics = article_topics + new_topics
	new_links = article_links + new_links
	...
	with open('output/event_topics.json', 'w') as f:
	...
	with open('output/event_links.json', 'w') as f:
		f.write('[')
		i = 0
